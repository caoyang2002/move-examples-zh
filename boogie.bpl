...
// fun prove::abortsIf0 [verification] at 18-hello_prover/sources/prove.move:9:5+83
procedure {:timeLimit 40} $42_prove_abortsIf0$verify(_$t0: int) returns ()
{
    ...
    // assume WellFormed($t0) at 18-hello_prover/sources/prove.move:9:5+1
    assume {:print "$at(2,127,128)"} true;
    assume $IsValid'u64'($t0);

    // trace_local[x]($t0) at 18-hello_prover/sources/prove.move:9:5+1
    assume {:print "$track_local(1,0,0):", $t0} $t0 == $t0;

    // $t1 := 0 at 18-hello_prover/sources/prove.move:10:18+1
    assume {:print "$at(2,168,169)"} true;
    $t1 := 0;
    assume $IsValid'u64'($t1);

    // $t2 := ==($t0, $t1) at 18-hello_prover/sources/prove.move:10:15+2
    $t2 := $IsEqual'u64'($t0, $t1);

    // if ($t2) goto L1 else goto L0 at 18-hello_prover/sources/prove.move:10:9+44
    if ($t2) { goto L1; } else { goto L0; }

    // label L1 at 18-hello_prover/sources/prove.move:11:18+3
    assume {:print "$at(2,190,193)"} true;
L1:

    // $t3 := 0 at 18-hello_prover/sources/prove.move:11:18+3
    assume {:print "$at(2,190,193)"} true;
    $t3 := 0;
    assume $IsValid'u64'($t3);

    // trace_abort($t3) at 18-hello_prover/sources/prove.move:11:13+8
    assume {:print "$at(2,185,193)"} true;
    assume {:print "$track_abort(1,0):", $t3} $t3 == $t3;

    // goto L3 at 18-hello_prover/sources/prove.move:11:13+8
    goto L3;

    // label L0 at 18-hello_prover/sources/prove.move:12:10+1
    assume {:print "$at(2,203,204)"} true;
L0:

    // label L2 at 18-hello_prover/sources/prove.move:13:5+1
    assume {:print "$at(2,209,210)"} true;
L2:

    // assert Not(Eq<u64>($t0, 0)) at 18-hello_prover/sources/prove.move:15:9+17
    assume {:print "$at(2,240,257)"} true;
    assert {:msg "assert_failed(2,240,257): function does not abort under this condition"}
      !$IsEqual'u64'($t0, 0);

    // return () at 18-hello_prover/sources/prove.move:15:9+17
    return;

    // label L3 at 18-hello_prover/sources/prove.move:13:5+1
    assume {:print "$at(2,209,210)"} true;
L3:

    // assert Eq<u64>($t0, 0) at 18-hello_prover/sources/prove.move:14:5+48
    assume {:print "$at(2,215,263)"} true;
    assert {:msg "assert_failed(2,215,263): abort not covered by any of the `aborts_if` clauses"}
      $IsEqual'u64'($t0, 0);

    // abort($t3) at 18-hello_prover/sources/prove.move:14:5+48
    $abort_code := $t3;
    $abort_flag := true;
    return;
...
// fun prove::plus1 [verification] at 18-hello_prover/sources/prove.move:2:5+42
procedure {:timeLimit 40} $42_prove_plus1$verify(_$t0: int) returns ($ret0: int)
{
    ...
    // bytecode translation starts here
    // assume WellFormed($t0) at 18-hello_prover/sources/prove.move:2:5+1
    assume {:print "$at(2,25,26)"} true;
    assume $IsValid'u64'($t0);

    // trace_local[x]($t0) at 18-hello_prover/sources/prove.move:2:5+1
    assume {:print "$track_local(1,1,0):", $t0} $t0 == $t0;

    // $t1 := 1 at 18-hello_prover/sources/prove.move:3:11+1
    assume {:print "$at(2,60,61)"} true;
    $t1 := 1;
    assume $IsValid'u64'($t1);

    // $t2 := +($t0, $t1) on_abort goto L2 with $t3 at 18-hello_prover/sources/prove.move:3:10+1
    ...
    // trace_return[0]($t2) at 18-hello_prover/sources/prove.move:3:9+3
    assume {:print "$track_return(1,1,0):", $t2} $t2 == $t2;

    // label L1 at 18-hello_prover/sources/prove.move:4:5+1
    assume {:print "$at(2,66,67)"} true;
L1:

    // assert Eq<u64>($t2, Add($t0, 1)) at 18-hello_prover/sources/prove.move:6:9+22
    assume {:print "$at(2,93,115)"} true;
    assert {:msg "assert_failed(2,93,115): post-condition does not hold"}
      $IsEqual'u64'($t2, ($t0 + 1));

    // return $t2 at 18-hello_prover/sources/prove.move:6:9+22
    $ret0 := $t2;
    return;

    // label L2 at 18-hello_prover/sources/prove.move:4:5+1
    assume {:print "$at(2,66,67)"} true;
L2:

    // abort($t3) at 18-hello_prover/sources/prove.move:4:5+1
    assume {:print "$at(2,66,67)"} true;
    $abort_code := $t3;
    $abort_flag := true;
    return;
